...
import com.yangf.pub_libs.util.GraphicsSettingsUtil;
 ...
 * @Data：
 * 自定义ImageView，加载网络图片，在使用该方法时需要添加网络请求权限，使用时需要添加URL完整地址
 * 该类已经废弃->采用Universal-Image-Loader框架用于加载网络图片
 * 可缓存内存与虚拟磁盘
 * @JDK: VERSION_1_8
...
public class UrlImageView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    public void setImageURL(String url,int bitmapWidth,int bitmapHeight) {
        ...
        Observable<Bitmap> observable = Observable.create(emitter -> {
            Bitmap bitmap = GraphicsSettingsUtil.getOkhttpBitmap(url);
            //如果获取不为null，则将其剪拆
            if (bitmap != null) {
                bitmap = GraphicsSettingsUtil.zoomBitmap(bitmap, bitmapWidth, bitmapHeight);
